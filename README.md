...
# \## Proje Yapısıasync-repo-challenge/
...
# │   └── test\_repository.py # Repository testleri
...
# ```bashgit clone https://github.com/esma6/async-repo-challenge.git
...
# `app/database.py` içinde `os.getenv("DATABASE\_URL")` çağrısıyla okunur.
...
# | `DATABASE\_URL` | Async SQLAlchemy bağlantı URL'i | `sqlite+aiosqlite:///./dev.db` |

# 

# PostgreSQL kullanmak için `.env` dosyasında:DATABASE\_URL=postgresql+asyncpg://user:password@localhost:5432/mydb
...
# \### 1. `Base.metadata.create\_all` (seçilen yöntem)

# 

# `app/database.py` içindeki `init\_db()` fonksiyonu, SQLAlchemy modellerinden
...
# Bu challenge için \*\*`create\_all`\*\* tercih edildi çünkü:
...
# 1\. Tabloları oluşturur (`init\_db()`).
...
# | `list\_all()` | Tüm kullanıcıları döner |

# | `get\_by\_id(id)` | Tek kullanıcıyı id ile getirir |
...
# `tests/test\_repository.py` içindeki `test\_unique\_email\_rolled\_back` testi
...
# 1\. \*\*Setup:\*\* Yeni bir `AsyncEngine` oluşturulur, `Base.metadata.create\_all`
...
# 3\. \*\*Cleanup:\*\* Test sonunda `Base.metadata.drop\_all` ile tablolar silinir,
...
# \- \*\*`expire\_on\_commit=False`:\*\* Async'te commit sonrası nesne erişimi
